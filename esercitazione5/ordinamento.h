#pragma once /*questa funzione è chiamata dentro
sort_mix.h*/
...
template <typename T> /* l'ho modificata per farla agire per
riferimento, così è più veloce*/
void ins_sort(std::vector<T> &vec) {
    int j=1;
    int l=size(vec);
    while (j<l) {
        T key=vec[j];
